...
import { AirportRunways, DEFAULT_APPROACH } from "../src/game/airportRunways.js";
...
test("swept ring crossings collect at high speed, but misses, hovering and repeated passes do not", () => {
  ...
  const points = new FlightPoints(); points.start(frame()); points.gates = [g];
...
test("series multipliers, mint rings, ten-ring bonus and one-time mission bonuses use earned scores", () => {
  const points = new FlightPoints(); points.start(frame());
...
test("pause, loading, teleports, expired gates and trail resets cannot grant free points", () => {
  const points = new FlightPoints(); points.reset({ mode: "arcade" }); points.start(frame()); points.gates = [gate(10)];
...
test("Point hunt ends at exactly 180 active seconds, clips the final crossing and ignores later movement", () => {
  const points = new FlightPoints(); points.reset({ mode: "arcade" }); points.start(frame());
  ...
  points.reset({ mode: "free" }); points.start(frame()); points.update(181, frame()); assert(!points.finished);
});
...
test("every actual menu vehicle can collect rings with its own flight physics in every unrestricted mode", () => {
  ...
  for (const mode of POINT_MODES.filter(m => m !== "landing")) for (const [vehicle, spec] of Object.entries(specs)) {
    const plane = createVehicleController(52, 21, 1000, 0, spec);
    const points = new FlightPoints(); points.reset({ mode, vehicle });
...
test("landing points follow the runway glide path and keep the flare zone clear", () => {
  const airports = new AirportRunways(), runway = airports.get(DEFAULT_APPROACH), matrix = new Matrix4();
  const spec = specs.pa28;
  const start = runway.pose(0, runway.definition.threshold - 6000, 340);
  const plane = createVehicleController(start.lat * 180 / Math.PI, start.lon * 180 / Math.PI, start.height, runway.definition.heading * Math.PI / 180, spec);
  const points = new FlightPoints(); points.reset({ mode: "landing" });
  const current = () => pointFlightFrame({ plane, spec, mapMatrix: matrix, mode: "landing", runway });
  points.start(current());
  assert(points.gates.length > 0);
  for (const marker of pointMapMarkers(points.gates, matrix)) {
    const p = runway.coordinates({ lat: marker.lat * Math.PI / 180, lon: marker.lon * Math.PI / 180, height: marker.height });
    assert(Math.abs(p.x) < .01); assert(-p.z < runway.definition.threshold + runway.definition.aimingPoint - 299);
    assert(Math.abs(p.y - ((runway.definition.threshold + runway.definition.aimingPoint + p.z) * Math.tan(Math.PI / 60) + 3)) < .01);
  }
  const near = runway.pose(0, runway.definition.threshold + runway.definition.aimingPoint - 100, 8);
  Object.assign(plane, near); points.makeTrail(current()); assert.equal(points.gates.length, 0);
});
...
test("wind-aware trails are reachable with aircraft, drones and unsteerable balloons in strong crosswinds", () => {
  for (const vehicle of ["pa28", "sailplane", "drone", "balloon"]) for (const east of [-20, 20]) {
    ...
    const plane = createVehicleController(52, 21, 6000, 0, spec), points = new FlightPoints();
    const current = () => pointFlightFrame({ plane, spec, weather, mapMatrix: new Matrix4(), mode: "free" });
    points.start(current());
...
test("terrain samples lift generated gates above obstacles without moving them later", () => {
  ...
  const points = new FlightPoints(); const context = pointFlightFrame({ plane, spec: specs.pa28, mapMatrix: matrix, ground: () => 900 });
...
test("points use the actual gameplay guards and ignore pause/menu/map/crash/loading states", () => {
  ...
  const context = vm.createContext({ ...Object.fromEntries(flags.map(k => [k, false])), freeMap: { open: false }, spaceScene: { overview: false } });
...
test("gate rendering stays camera-relative, bounded and non-collidable, and disposes its resources", () => {
  const scene = new Scene(), visuals = new PointVisuals(scene), points = new FlightPoints(); points.start(frame());
