...
- **Traffic dodge** – survive 90 seconds among 50 nearby Boeing 737s and Airbus A320s. They cross your path and approach head-on, with new aircraft replacing those that pass. Bank, climb or descend to avoid hitting their fuselage, wings or tail. A collision ends the round; surviving the full countdown wins. The HUD shows the nearest aircraft and your best survival time for the selected vehicle. **M**, **Esc** and touch controls work as usual; the map and pause stop both traffic and the clock. Press **R** after a crash or a win to restart. This mode has no point rings and works independently of OpenSky traffic.
...
All fourteen vehicles collect points in Point hunt, Fly home and Guess the region. Free flight and Land have no rings, point scoring, points panel or ring markers on the map. Fly through a gold ring for **100 points**, or a mint ring for **200**. Consecutive pickups increase the multiplier every three rings, up to **×5**; every ten collected rings adds **500 points**. Missing a ring or waiting too long breaks the series. **New trail** places new rings ahead after a three-second cooldown, preserves points and resets the series.

Rings stay in fixed world positions. Their spacing follows the vehicle's speed, with wind-aligned trails for balloons, descending trails for sailplanes and vertical trails for ascending Falcons. Terrain available when generating a trail raises obstructed rings above the surface.

The points panel shows the next ring's distance and relative altitude. **M** and the minimap show ring locations in Point hunt. Fly home and Guess the region keep their map restrictions; follow the visible rings and relative arrow. Collection pauses during menus, loading, pause, map views and after a crash or completed round. In Point hunt, map views also pause the timer and vehicle. Teleports rebase the trail without awarding points for the jump.

Finding home adds **2,000 points**. A location guess adds up to **3,000**, decreasing with distance error. In modes with points, a successful touchdown adds **250–1,200**, depending on descent speed; a safe stop adds **250**. Each mission bonus is awarded once per flight. Records persist in this browser separately for each mode and vehicle; if storage is unavailable, records last for the session. Multiplayer rings are individual, and the **Tab** player list displays flight points separately from region wins.
...
After stopping, press **H** or **Call Cessna for tow**. The game checks 250 m of grass for the ground run and the climb corridor ahead. A Cessna 172 arrives and attaches a visible tow rope. While attached, **W/S**, **A/D**, the arrow keys or the touch stick control the Cessna, with the glider following on the cable. **AIRBRK** becomes **THR** for the Cessna: scroll or drag to change power, hold **Shift** for full power or **Ctrl** for idle, or use **,** / **.** for idle / full power. The tow starts at full power; neutral stick provides an assisted takeoff and gentle climb. **L** or **Release tow rope** disconnects it at any time; it releases automatically at 350 m above the departure field and flies away. **B** cancels a ground tow. Controls immediately return to the unpowered glider with airbrakes retracted. The tow aircraft, its bank and the cable are also visible to multiplayer peers; both buttons support touch input. The assisted tow compensates for wind; shared wind and thermals apply after release.
...
## Solar system and lunar flight

Choose **Single player → Free flight → SpaceX Falcon 9**, enter a city and start. The **Destination** selector offers Mercury, Venus, Earth, Mars, Jupiter, Saturn, Uranus, Neptune, the Sun and the Moon. **Fly to Moon** engages lunar landing guidance from your current position. Select **1000×** to accelerate the journey; the panel shows the effective rate, which automatically drops near either surface. **Stop guidance** returns to manual thrust and steering at 1×. `Esc` pauses the flight.
...
**Space map** offers **Earth & Moon**, **Entire solar system** and **Selected destination** views. Orbits and positions retain mean physical distances; planet markers are enlarged only in the full-system overview so they remain visible. The selected-body view restores physical size and follows the moving planet. Drag to orbit, scroll to zoom, and use `Esc` or **Return to flight** to return. `M` opens this view above 100 km in Free flight. Earth rotates once per sidereal day, with a 23.44° tilted axis in the overview. The ship marker and text labels are enlarged for legibility. **Magnetosphere & flight model** enables the optional field illustration and its solar-wind pressure control. [Scientific assumptions and references](docs/space-science.md) distinguish altitude from distance measured from Earth's centre.

Other destinations engage a fictional **cruise drive** for a continuous, roughly 55-second journey. The route leaves the departure body radially and detours around the Sun and planets. Arrival holds an observation orbit; choose another destination, inspect it in the space map or use **Leave orbit · manual flight**. **Stop guidance** disengages the cruise drive and returns to a local drift. Mission time acceleration applies to local flight and observation orbits; cruise itself runs at 1×. Gas giants and the Sun are observation destinations, not landing surfaces. Saturn and Uranus have visible rings.

The Moon has geometric crater bowls, raised rims, rolling terrain and close-up regolith detail. The renderer, altimeter and collision detector sample the same deterministic body-fixed height field. A gentle, upright touchdown stays attached to the moving Moon. **Surface camera** gives a wider view of the surrounding craters, and **Surface light** reveals the relief even on the night side. **Take off from Moon** lets you explore manually or depart for another planet. NASA LROC imagery supplies the broad colour map; the relief is procedural and does not claim surveyed elevations.

Falcon 9 is an **assisted game spacecraft with unlimited propellant**. Fuel consumption, life support, radiation damage and a real Falcon lunar mission profile are not simulated. The Moon follows a mean circular inclined orbit and a mission-relative clock, rather than the current lunar ephemeris. Earth's rotation and sunlight follow real UTC independently of mission time warp. The eight planets follow illustrative circular heliocentric orbits, transformed into the same Earth-centred equatorial coordinates used for the Moon. Dragon's atmospheric parachute is available only below 80 km above Earth.
...
The aircraft rests on its wheels, with animated retractable gear, wheel rotation and suspension compression. Landings allow a modest margin for mistakes: bank up to 10°, heading error up to 17°, pitch from −2.5° to 13° relative to the runway, and speed between 60% and 150% of the aircraft's approach target. Sink above 4.2 m/s causes a reduced bounce; above 6 m/s the gear fails. A near-level nose-wheel brush is recoverable if the main wheels are within 65 cm of the pavement. A belly landing, steep nose-first impact, wing strike or departure from the pavement still ends the flight. `R` restarts the approach after a crash. Landing gear cannot retract while on the ground.
...
New contacts accept positions up to 90 seconds old. Between observations, trajectories use speed, heading, vertical rate and recent changes in direction and speed; missing motion fields can be inferred from position history. Contacts keep moving through data gaps for up to 180 seconds, begin fading after 120 seconds and are removed from memory after 240 seconds. Positions beyond 30 seconds are labelled **Estimated**; cached responses do not reset their age. New reports blend from the currently displayed position and heading, and clock corrections are gradual. The displayed uncertainty is a growing heuristic, not a measured accuracy guarantee. This does not increase OpenSky polling or credit use.

Aligned descent near a known runway is labelled **Likely approach** or **Likely final**, with the airport and runway shown in the traffic panel. Consecutive aligned reports can predict a flare and slowing rollout; **Predicted rollout** is always an estimate, never confirmation of a landing. Climbing away cancels the landing forecast. Pressure altitude alone is insufficient to predict touchdown. A provider-reported ground contact removes the aircraft. Outside known runways the forecast has no local terrain model, and missing data cannot establish a flight's intent or actual landing.

Approach detection uses the bundled, public-domain [OurAirports](https://ourairports.com/data/) runway snapshot in `public/data/traffic-runways.json`. It covers open airport runways at least 800 m long with usable threshold coordinates, heading and elevation, including displaced thresholds. It is not live operational data. Regenerate it with `python3 scripts/import-traffic-runways.py airports.csv runways.csv` after downloading the two CSV files from OurAirports. An enabled transponder alone does not guarantee OpenSky coverage. `geo_altitude` is preferred; pressure altitude is a labelled fallback and may differ from terrain height.
